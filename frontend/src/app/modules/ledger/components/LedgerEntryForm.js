...
export default function LedgerEntryForm({
  ...
  const handleAllocationToggle = (bill, isChecked) => {
    ...
    if (isChecked) {
      ...
      const amountToApply =
        creditAmount > 0
          ? Math.min(bill.balanceDue, remainingCredit)
          : bill.balanceDue;
      currentAlloc.push({
  ...
  return (
    <div
      className={`bg-white border rounded-3xl shadow-sm overflow-hidden transition-colors ${editingId ? "border-blue-300 shadow-blue-100" : "border-slate-200"}`}
    >
      <div
        className={`p-4 border-b flex justify-between items-center ${editingId ? "bg-blue-50/50 border-blue-100" : "bg-slate-50/50 border-slate-100"}`}
      >
        <h2
          className={`font-bold flex items-center gap-2 ${editingId ? "text-blue-700" : "text-slate-900"}`}
        >
          {editingId ? (
      ...
      <form onSubmit={onSubmit} className="p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-6 gap-5">
          <InputField
            className={isDebitTab ? "sm:col-span-1" : "sm:col-span-2"}
            label="Date"
            type="date"
            name="date"
            required
          ...
          />
          <InputField
            className={isDebitTab ? "sm:col-span-3" : "sm:col-span-4"}
            label="Description"
            name="desc"
            required
            placeholder={
              isDebitTab ? "e.g. Sales Invoice" : "e.g. Payment Received"
            }
            value={formData.desc}
            onChange={onChange}
          />
          {isDebitTab && (
            <InputField
              ...
              label={isDebitTab ? "Invoice No." : "Ref No."}
              name="ref"
          ...
          {isDebitTab && (
            <InputField
              className="sm:col-span-6 font-bold"
                ...
                <div className="sm:col-span-6 bg-slate-50 border border-slate-200 rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-sm text-slate-800 flex items-center gap-1.5">
                      <Receipt size={16} className="text-blue-600" /> Apply
                      Payment to Open Invoices
                    </h3>
                    <div className="text-xs font-bold text-slate-500">
                      Unallocated:{" "}
                      <span
                        className={
                          unallocatedAmount > 0 ? "text-purple-600" : ""
                        }
                      >
                        {safeFormatCurrency(unallocatedAmount)}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-2 max-h-48 overflow-y-auto customScroller pr-2">
                    {unpaidInvoices.map((bill) => {
                      ...
                      const allocatedObj = allocations.find(
                        (a) => a.billId === billId,
                      );
                      ...
                      return (
                        <div
                          key={billId}
                          className={`flex items-center justify-between p-3 rounded-xl border transition-colors ${isSelected ? "bg-white border-blue-200 shadow-sm" : "bg-transparent border-slate-200 hover:bg-white"}`}
                        >
                          <label className="flex items-center gap-3 cursor-pointer flex-1">
                            <input
                              ...
                              checked={isSelected}
                              onChange={(e) =>
                                handleAllocationToggle(bill, e.target.checked)
                              }
                            ...
                            <div>
                              ...
                              <p className="text-xs font-medium text-red-500">
                                Due: {safeFormatCurrency(bill.balanceDue)}
                              </p>
                            </div>
                          </label>
                          {isSelected && (
